...
###   This script attempts to download the signature file SHA256SUMS.asc from heptacoin.org
...
WORKINGDIR="/tmp/heptacoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://heptacoin.org/bin/"
VERSIONPREFIX="heptacoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'heptacoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Heptacoin Core binary release signing key installed?"
   fi
